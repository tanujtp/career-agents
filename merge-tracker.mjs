...
for (const file of tsvFiles) {
  ...
  if (duplicate) {
    ...
    if (newScore > oldScore || (newScore === oldScore && duplicate.pdf === '❌' && addition.pdf === '✅')) {
      console.log(`🔄 Update: #${duplicate.num} ${addition.company} — ${addition.role} (${oldScore}→${newScore})`);
      const lineIdx = appLines.indexOf(duplicate.raw);
      if (lineIdx >= 0) {
        // If the addition has a ✅ for PDF, use it. Otherwise keep the duplicate's PDF status.
        const mergedPdf = (addition.pdf === '✅' || duplicate.pdf === '✅') ? '✅' : '❌';
        // If score is same but we're updating PDF, just use a different note
        const notePrefix = newScore > oldScore ? `Re-eval ${addition.date} (${oldScore}→${newScore}).` : `Re-eval ${addition.date} (PDF Gen).`;
        const updatedLine = `| ${duplicate.num} | ${addition.date} | ${addition.company} | ${addition.role} | ${addition.score} | ${duplicate.status} | ${mergedPdf} | ${addition.report} | ${notePrefix} ${addition.notes} |`;
